    ...
    try {
      ...
      if (group.requiresApproval) {
        ...
        const existingRequest = group.joinRequests.find(
          (req) =>
            req.user.toString() === userId.toString() &&
            req.status === "pending"
        );
...
router.post("/:groupId/leave", authMiddleware, async (req, res) => {
  try {
    ...
    if (group.creator.toString() === userId.toString()) {
      return res.status(400).json({
        message:
          "Group creator cannot leave. Please transfer ownership or delete the group.",
      });
    }
  ...
  async (req, res) => {
    try {
      ...
      const similarUsers = await User.find({
        ...
        .select(
          "name university subjects studyGoals experienceLevel studyStyle availability"
        )
        .limit(20);
      ...
      req.user.subjects.forEach((subject) => {
        const usersForSubject = similarUsers.filter((user) =>
