...
router.get("/profile/:userId", authMiddleware, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .select(
        "name university year major subjects studyGoals experienceLevel studyStyle createdAt"
      )
      .populate("studyGroups", "name subject");
  ...
  async (req, res) => {
    try {
      ...
      const users = await User.find(query)
        .select(
          "name university year major subjects studyGoals experienceLevel studyStyle"
        )
        .limit(20)
        .sort({ createdAt: -1 });
...
router.get("/stats", authMiddleware, async (req, res) => {
  try {
    ...
    const activeGroups = user.studyGroups.filter(
      (group) => group.status === "active"
    );
    const completedGroups = user.studyGroups.filter(
